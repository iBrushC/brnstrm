// LLM export — a deterministic formatter that condenses a board (or a single
// section / note) into a coding-ready prompt and copies it to the clipboard.
//
// Three tiers, matching the three things you can export:
//   • board   — every section, note, and arrow on the current board
//   • section — one section: its nested subsections, notes, and inner arrows
//   • note    — a single note: basically just its content
//
...
// export time so the output reflects exactly what the user laid out.
...
function formatBoard(boardName, { nodes, sections, connections }) {
...
function formatSection(sectionId, { nodes, sections, connections }) {
  const nameOf = makeNameOf(nodes, sections);
